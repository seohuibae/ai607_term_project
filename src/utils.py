...
def construct_gt_negative(edge_index, gt_negative, ratio, num_nodes, k, device):
    ...
    num_gt = int(k * ratio) # number of gt ratio 
    num_gt = min(num_gt, len(gt_negative))
    neg_gt_idxs = torch.randint(0, len(gt_negative), num_gt)
    gt_negative = list(gt_negative.items())[neg_gt_idxs]
    gt_negative = [item[1] for item in gt_negative]
    gt_negative = [torch.tensor([item[0], item[1]]) for item in gt_negative]
    gt_negative = torch.cat(gt_negative, dim=0)
    ...
    neg_edge_index = torch.cat([neg_edge_index, gt_negative])
